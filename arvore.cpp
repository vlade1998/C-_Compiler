...
void generateSpecialFunction(treeNode* tree){
	...
	}else if(tree->name == "kernelMode"){
		string regDataAddress = "_t" + to_string(tempIndex%16);
		tempIndex++;
		insertQuad("immed", "0", " ", regDataAddress);
		insertQuad("loadRegKernel", regDataAddress, " ", " ");
	}
}
